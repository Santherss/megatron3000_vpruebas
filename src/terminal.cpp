...
bool crear_tabla(char *str, DiscoFisico *disk)
{
    ...
    char cabecera[512];
    if (!agregar_a_esquema(disk, nombre, archivo, sep, cabecera))
        return false;
    int tamanos[20];
    int i = 0;
    int idx = 1;
...
bool evaluarCondicion(char *valorCampo, char *operador, char *valorCondicion, char *tipoDato)
{
    ...
    {
        if (compararTotal(operador, "=="))
            return compararTotal(valorCampo, valorCondicion);
    }
...
bool evaluarCondiciones(char *linea, char *condiciones, char campos[][50], char tipos[][10], int n_campos)
{
    ...
    while (*ptr)
    {
        ...
        if (!evaluarCondicion(valorCampo, operador, valor, tipoDato))
            return false;
    }
...
bool procesar_select(char *str, char *mayus, DiscoFisico *mydisk)
{
    ...
    if (!compararTotal(columnas, "*"))
    {
        ...
        while (tok && n_seleccionadas < 50)
        {
            ...
            while (*tok == ' ')
                tok++; // espacios al inicio
            char *end = tok + strlen(tok) - 1;
            while (end > tok && *end == ' ')
                *end-- = '\0'; // espacios al final
    ...
    {
        int i = 0;
        while (relacion[pos] != '#' && relacion[pos])
            campos[n_campos][i++] = relacion[pos++];
        campos[n_campos][i] = '\0';
        pos++;

        i = 0;
        while (relacion[pos] != '#' && relacion[pos])
            tipos[n_campos][i++] = relacion[pos++];
        tipos[n_campos][i] = '\0';
        pos++;
                        ...
                        if (idx >= 0 && idx < num_campos_linea)
                        {
                            ...
                            for (int i = 0; i < tamano(campos_linea[idx]); i++)
                            {
                                if (campos_linea[idx][i] != '~')
                                {
                                    ...
                                    if (salida)
                                        fputc(campos_linea[idx][i], salida);
                                }
                            }
    ...
    if (salida)
    {
        fclose(salida);
        char nulle[250];
        if (!agregar_a_esquema(mydisk, archivo_salida, archivo_salida, '|', nulle))
            return false;
        // Buscar sector libre desde (0,0,0,1)
        unsigned int d = 0, p = 0, s = 1;
        int c = 0;
...
int procesar_consulta(char *str, DiscoFisico *mydisk)
{
    ...
    if (estaLiteral(mayus, palabras_salida))
    { // salir
        mydisk->reporte();
        printf("\n[+] Saliendo...\n");
        if (bufferManager)
    ...
    if (buscar((char *)palabras_adicionales[2], mayus) == 0)
    { // SELECT-DISCO
    ...
    if (buscar((char *)palabras_adicionales[1], mayus) == 0)
    { // createdisco
    ...
    if (compararTotal((char *)palabras_reservadas[0], mayus))
    { // HELP
    ...
    if (buscar((char *)palabras_reservadas[5], mayus) == 0)
    { // CREATE tabla archivo separador
        crear_tabla(str, mydisk);
        return 1;
    }

    if (buscar((char *)palabras_reservadas[1], mayus) == 0)
    { // SELECT
        // printf("%s\n",ruta_base.c_str());
    ...
    if (buscar((char *)palabras_reservadas[6], mayus) == 0)
    { // insert 32 tabla Arhchivo.txt separador
        printf("insert\n");
    ...
    if (buscar("ESQUEMA", mayus) == 0)
    { // esquema, para ver nuestras relaciones
    ...
    if (buscar((char *)palabras_adicionales[0], mayus) == 0)
    { // reporte
        mydisk->reporte();
        // printf("reporanrrr\n");
        return 1;
    ...
    if (buscar("BLOQUE-DISCO", mayus) == 0)
    {
        ...
        {
            if (!mydisk->encontrarSector(ruta, num_id, ii))
                return false;
